...
{
    public GameObject MainMenuElementPrefab;
    ...
    private void Start()
    {
        ...
        {
            GameObject newObject = Instantiate(MainMenuElementPrefab, leftColumn.transform);
            newObject.GetComponent<MainMenuElementScript>().Initialize(tempName, menuData[tempName], rightColumn);
            newObject.name = tempName;
            listOfElements.Add(newObject);
        }
    ...
    public Tuple<int, int> ShiftCursor(Tuple<int, int> _cursorPosition, AdjacentDirection direction)
    {
        ...
        y = CustomMath.CustomMath.positiveMod(y, listOfElements.Count);
        x = CustomMath.CustomMath.positiveMod(x, 2);
        ...
        if (y != _cursorPosition.Item2)
        {
            switch (x)
            {
                case 0:
                    ...
                    SetElementActive(returnValue, true);
                    break;
                case 1:
    ...
    {
        listOfElements[_position.Item2].GetComponent<MainMenuElementScript>().Active = _isActive;
    }
    ...
    public void SetMenuActive(bool _isActive)
    {
        ...
        {
            isSetup = false;
            listOfElements[0].GetComponent<MainMenuElementScript>().Active = true;
            rightColumn.transform.Find("Text").GetComponent<Text>().material = GameManager.instance.GUIManager.UITextDarkenedMaterial;
        }
    }
    ...
    {
        foreach (GameObject tempGameObject in listOfElements)
        {
            tempGameObject.GetComponent<CommandMenuElementScript>().Foreground = _foreground;
        }
        rightColumn.transform.Find("Text").GetComponent<Text>().material = GameManager.instance.GUIManager.UITextDarkenedMaterial;
    }
